...
public class Main {
    ...
    public static void master(Polynomial p1, Polynomial p2, int processes){
        ...
        int length = p1.getSize() / (processes - 1); //calculate how many coeficients each process will get. Rank 0 not participating
        ...
        Object[] results = new Object[processes - 1]; //prepare array of resultd
        for (int i = 1; i  < processes; i++){
            MPI.COMM_WORLD.Recv(results, i - 1, 1, MPI.OBJECT, i, 0);//collects the result from each worker
        }

        Polynomial result = Utils.getResult(results); //combine the results
    ...
    public static void regularWorker(int me){
        //arrays to hold the polynomial objects from master
        Object[] p1 = new Object[2];
        Object[] p2= new Object[2];
        //arrays to receive the segment this worker is responsible for
        int[] start = new int[1];
        int[] end = new int[1];

        //receive polynomial data
    ...
    public static void karatsubaWorker(int me){
        ...
        //setting coefficients outside of range to 0

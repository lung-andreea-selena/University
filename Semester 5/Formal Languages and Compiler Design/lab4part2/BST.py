...
class BinarySearchTree:
    ...
    def inorder_traversal_write(self, filename):
        f = open(filename, "w", encoding='utf-8')
        return self._inorder_recursive_write_(self.root, f)

    def _inorder_recursive_write_(self, root, f):
        if root is not None:
            self._inorder_recursive_write_(root.left, f)
            f.write(str(self.get_index(root.key)) + " : " + root.key + "\n")
            self._inorder_recursive_write_(root.right, f)
